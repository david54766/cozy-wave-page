...
function Landing() {
  ...
  // In the native app the marketing landing page isn't the entry point —
  // send users straight to /auth (which itself forwards signed-in users on to
  // the dashboard). Rendering nothing until the redirect avoids a flash of the
  // web landing page behind the splash screen.
  const isNative = Capacitor.isNativePlatform();
  useEffect(() => {
    if (isNative) navigate({ to: "/auth", replace: true });
  }, [isNative, navigate]);
  if (isNative) return <div className="min-h-screen bg-background" />;
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-24 sm:py-32 text-center">
          <BrandLogo className="mx-auto h-40 sm:h-56 w-auto mb-6" />
      ...
      <footer className="border-t border-border/60">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between text-sm text-muted-foreground">
          <span>© {new Date().getFullYear()} Alpha Gamma Alpha · Managed by Classroom Panda LLC</span>
